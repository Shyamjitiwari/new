...
class OneDayBeforeTaskClassReminder extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'onedaybefore:taskclassreminder';
     ...
     * @var string
     */
    protected $description = 'This command sends a reminder to parents one day before the class.';
                ...
                if($dateOfTomorrow == $dateOfTaskClass){
                    ...
                    foreach($users as $user){
                        ...
                        if($role == "student"){
                            ...
                            if($isLocationOnline){
                                $message = __('sms_and_email.one_day_before_class_reminder_for_online_class',['Name' => $user->full_name, 'classDate' => $classDate, 'classTime' => $classTime, 'timeZone' => $user_timezone,'rescheduleLink' => $reschedule_link]);
                                $message = stripslashes($message);
                            }
                            else{
                                $location_address = Location::where('id', $location_id)->value('address');
                                $message = __('sms_and_email.one_day_before_class_reminder_for_offline_class',['Name' => $user->full_name, 'classDate' => $classDate, 'classTime' => $classTime,'timeZone' => $user_timezone, 'Location' => $location_address,'rescheduleLink' => $reschedule_link]);
                                $message = stripslashes($message);
                            }
                            if($emailAddress != null && $emailAddress != ""){
                                ...
                                //$mail->send_class_reminder("rida@codewithus.com","One day before Class reminders",$data); 
