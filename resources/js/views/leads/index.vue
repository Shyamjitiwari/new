<template>
    <div>
        <page-title
            :title="'Lead (Count : ' + this.getLeadsCount + ')'"
...
<!--                        </div>-->
        ...
        <div class="row">
            <div class="col">
                            ...
                            <table class="table table-sm">
                                ...
                                <tr>
                                   ...
                                   <tbody class="collapse multi-collapse" id="dailyReports">
                                           ...
                                           <tr v-bind:class="[lead.lead_history[0].count > 3 ? 'font-weight-bolder bg-danger' : 'font-weight-bolder bg-warning']"  v-for="lead in getLeadsForToday" :key="lead.id" v-if="getLeadsForToday.length">
                                    <td></td>
                                    <td>
                                ...
                                <tbody>
                                <tr
                                    v-for="lead in getLeads.data" :key="lead.id"
                                ...
                                >
                                    ...
                                    <td v-if="hasPermission['role'] === 1 || hasPermission['role'] === 2 || hasPermission['role'] === 3">
                                        <span v-if="!lead.parent_id && lead.created_by" class="text-capitalize" @dblclick="dblClickSearch(lead.created_by.name, 'lead_owner')">
                                            {{lead.created_by.name}}
                                            <!-- v-if="hasPermission['transfer-owner-leads']" -->
                                            <span ><i class="material-icons text-warning" @click="changeLeadOwner(lead)">edit</i></span>
                                        </span>
                                        <span v-else-if="lead.parent_id && lead.parent.created_by" class="text-capitalize" @dblclick="dblClickSearch(lead.parent.created_by.name, 'lead_owner')">
                                            {{lead.parent.created_by.name}}
                                            <!-- v-if="hasPermission['transfer-owner-leads']" -->
                                            <span ><i class="material-icons text-warning" @click="changeLeadOwner(lead.parent)">edit</i></span>
                                        </span>
                                        <span v-else>-</span>
                                    </td>
                                    <td>
                ...
                <br />
                <pagination show=4 :data="getLeads" @updatePagination="updatePagination"> </pagination>
            </div>
        </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                click_links : [
                    {click_event: 'importLeads', click_hover : 'Import Leads', type : 'icon', icon:'attendance', class : 'text-success'},
                ],
                lead: {},
                pagination: {},
                search : {
        ...
        computed: {
            ...mapGetters(["getLeads", "getErrors", "getSearchFilter", 'getAuth', 'getLeadsCount', 'hasPermission', 'getLeadsForToday']),

            reload(){
                return {type : 'fetchLeads', payload : {next_page: this.getLeads.current_page, search: this.search}};
            },
            showBulkAction(){
